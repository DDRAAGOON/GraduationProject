...
class RecruitmentJobDetailsScreen extends StatefulWidget {
  ...
  @override
  State<RecruitmentJobDetailsScreen> createState() =>
      _RecruitmentJobDetailsScreenState();
}

class _RecruitmentJobDetailsScreenState
    extends State<RecruitmentJobDetailsScreen> {
              ...
              children: [
                ...
                ...reasons.map(
                  (reason) => RadioListTile<String>(
                    ...
                    onChanged: (val) =>
                        setModalState(() => _selectedReportReason = val),
                  ),
                ),
                          ...
                          if (_selectedReportReason == null ||
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  isAr
                                      ? 'يرجى إكمال بيانات الإبلاغ'
                                      : 'Please complete report details',
                                ),
              ...
              children: [
                    ...
                    children: [
                      Container(
                        ...
                        child: widget.job.companyLogoUrl != null
                            ...
                            : Icon(
                                widget.job.logoIcon ?? Icons.business,
                                color: Colors.white,
                                size: 30,
                              ),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              _cleanTitle(
                                t.translateJobTitle(widget.job.title),
                              ),
                              style: const TextStyle(
                                fontSize: 18,
                                fontWeight: FontWeight.bold,
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: Text(
                                    translateValue(widget.job.type, isAr),
                                    style: const TextStyle(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildInfoRow(
                        ...
                        isAr ? 'تاريخ النشر' : 'Posted Date',
                        '${widget.job.publishedAt.year}-${widget.job.publishedAt.month.toString().padLeft(2, '0')}-${widget.job.publishedAt.day.toString().padLeft(2, '0')}',
                      ),
                      _buildInfoRow(
                        Icons.location_on_outlined,
                        isAr ? 'مكان العمل' : 'Location',
                        translateValue(widget.job.location, isAr),
                      ),
                      _buildInfoRow(
                        Icons.work_outline,
                        isAr ? 'نوع الوظيفة' : 'Job Type',
                        translateValue(widget.job.type, isAr),
                      ),
                      _buildInfoRow(
                        Icons.calendar_month_outlined,
                        isAr ? 'أيام العمل' : 'Work Days',
                        isAr ? '6 أيام في الأسبوع' : '6 Days / Week',
                      ),
                      _buildInfoRow(
                        Icons.attach_money,
                        isAr ? 'الراتب' : 'Salary',
                ...
                _buildSectionCard(
                  ...
                  child: SizedBox(
                    width: double.infinity,
                    child: Text(
                      translateValue(widget.job.category, isAr),
                      style: const TextStyle(
                        color: Colors.white,
                        fontSize: 15,
                        fontWeight: FontWeight.bold,
                      ...
                      if (_isLoadingReviews)
                        ...
                        ..._reviews.map((review) {
                          return _buildReviewItem(
                            {
                              'userName': RatingUtils.authorName(review),
                              'rating': RatingUtils.value(review).round(),
                              'comment': RatingUtils.comment(review),
                              'date': RatingUtils.date(review) != null
                                  ? RatingUtils.formatRelativeDate(
                                      RatingUtils.date(review)!,
                                      isAr: isAr,
                                    )
                                  : '',
                            },
  ...
  Widget _buildSimpleCapacityBar(BuildContext context, RecruitmentJob job) {
    return Column(
      ...
      children: [
        Text(
          t.tr(
            en: 'Accepted ${job.acceptedCount} of ${job.capacity}',
  ...
  Widget _buildRatingBox() {
    final average = RatingUtils.average(_reviews);
    final label = _reviews.isEmpty ? '--' : average.toStringAsFixed(1);
    return Container(
