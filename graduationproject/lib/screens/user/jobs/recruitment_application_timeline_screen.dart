...
class RecruitmentApplicationTimelineScreen extends StatefulWidget {
  ...
  @override
  State<RecruitmentApplicationTimelineScreen> createState() =>
      _RecruitmentApplicationTimelineScreenState();
}

class _RecruitmentApplicationTimelineScreenState
    extends State<RecruitmentApplicationTimelineScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredApps = allApps.where((a) {
      ...
      if (_selectedStatusTab == 'Applied') {
        matchesTab = status.contains('appli') || a.status == 'تم التقديم';
      } else if (_selectedStatusTab == 'Hired')
        matchesTab = status.contains('hire') || a.status == 'تم التوظيف';
      else if (_selectedStatusTab == 'In Review')
        matchesTab = status.contains('review') || a.status == 'قيد المراجعة';
      else if (_selectedStatusTab == 'Declined')
        matchesTab =
            status.contains('reject') ||
            status.contains('decline') ||
            a.status == 'مرفوض';
      else if (_selectedStatusTab != 'All')
        matchesTab = a.status == _selectedStatusTab;

      bool matchesSearch =
          a.jobTitle.toLowerCase().contains(_searchQuery) ||
        ...
        children: [
              ...
              children: [
                Row(
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        isAr
                            ? '${store.currentUserName} ،استمر في العمل الجيد'
                            : 'Keep up the good work, ${store.currentUserName}',
                ...
                Text(
                  isAr
                      ...
                      : 'Here is what\'s happening with your apps as of today',
                  style: TextStyle(
                    color: Theme.of(
                      context,
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  flex: 3,
                  child: Text(
                    isAr ? 'مقدم الخدمة / الشركة' : 'Company',
                    style: const TextStyle(
                      color: Colors.grey,
                      fontSize: 11,
                      fontWeight: FontWeight.bold,
          ...
          Expanded(
            child: filteredApps.isEmpty
                ? Center(
                    child: Text(isAr ? 'لا توجد نتائج' : 'No matching results'),
                  )
                : ListView.builder(
                    padding: const EdgeInsets.symmetric(
                      horizontal: 20,
